...
export const statsHandler: Handler = {
  async handle(event: APIGatewayEvent): Promise<APIGatewayResponse> {
    try {
      ...
      const [usersCount, placesCount, reviewsCount, photosCount, pendingReviews, recentActivity] = await Promise.all([
        // Total users
        db.selectFrom('users')
        ...
        db.selectFrom('location_addresses')
          .select(sql<number>`count(*)::int`.as('count'))
      ...
      const activity = recentActivity.rows[0] as { new_users: number; new_reviews: number; new_photos: number };
