...
async function up(db) {
  ...
  await sql`CREATE INDEX idx_context_eb_hnsw ON restaurants USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 200);`.execute(
    db
  );
  ...
  await sql`CREATE INDEX idx_restaurants_name_trgm ON restaurants USING gin (f_unaccent(name_vi) gin_trgm_ops);`.execute(
    db
  );

  await sql`CREATE INDEX idx_restaurants_addr_trgm ON restaurants USING gin (f_unaccent(address) gin_trgm_ops);`.execute(
    db
  );
