...
# Custom policy: Secrets + S3 + Rekognition + DB access
...
resource "aws_security_group" "lambda" {
  name        = "${var.project_name}-rekognition-lambda-sg-${var.environment}"
  description = "Security group for Rekognition Lambda"
  vpc_id      = var.vpc_id
...
resource "aws_security_group_rule" "lambda_to_rds" {
  ...
  description              = "Allow Rekognition Lambda to connect to RDS"
}
