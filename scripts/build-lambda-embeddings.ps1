# Build Lambda Embeddings package với Docker

$ErrorActionPreference = "Stop"

$ROOT_DIR = Split-Path -Parent $PSScriptRoot
$LAMBDA_DIR = "$ROOT_DIR\infrastructure\terraform\modules\lambda-embeddings"
$SRC_DIR = "$LAMBDA_DIR\src"
$BUILD_DIR = "$LAMBDA_DIR\build"
$ZIP_FILE = "$LAMBDA_DIR\lambda-embeddings.zip"
...
Write-Host "`n==> Building Lambda Embeddings package với Docker..." -ForegroundColor Cyan
...
$buildOutput = & docker build --progress=plain -t lambda-embeddings-builder:latest . 2>&1
...
$CONTAINER_ID = docker create lambda-embeddings-builder:latest
