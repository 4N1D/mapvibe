# Build Lambda OCR Menu package với Docker

$ErrorActionPreference = "Stop"

$ROOT_DIR = Split-Path -Parent $PSScriptRoot
$LAMBDA_DIR = "$ROOT_DIR\infrastructure\terraform\modules\lambda-ocr-menu"
$SRC_DIR = "$LAMBDA_DIR\src"
$BUILD_DIR = "$LAMBDA_DIR\build"
$ZIP_FILE = "$LAMBDA_DIR\lambda-ocr-menu.zip"
...
Write-Host "`n==> Building Lambda OCR Menu package với Docker..." -ForegroundColor Cyan
...
$buildOutput = & docker build --progress=plain -t lambda-ocr-menu-builder:latest . 2>&1
...
$CONTAINER_ID = docker create lambda-ocr-menu-builder:latest
